...
function WorkOrderCard({ onUpdated, order, token }: { onUpdated: (order: Partial<WorkOrder> & { id: string }) => void; order: WorkOrder; token: string }) {
  const [action, setAction] = useState<'failed' | 'reschedule_required' | null>(null)
