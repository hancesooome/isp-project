  ...
  return (
    <div className="min-h-screen bg-[#0a0d12] text-slate-100">
      ...
      <div className="min-h-screen md:pl-64">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-white/8 bg-[rgba(10,13,18,0.82)] px-4 backdrop-blur-xl md:hidden">
          <PortalBrand compact />
          <details className="group relative">
            <summary className="grid size-11 cursor-pointer list-none place-items-center rounded-[10px] border border-white/10 bg-white/6 text-sm font-semibold text-white transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400">
              <span className="sr-only">Open account menu</span>
              <span aria-hidden="true">{getInitial(email)}</span>
            </summary>
            <div className="absolute right-0 mt-2 w-72 rounded-[14px] border border-white/10 bg-[#161c26] p-3 shadow-2xl">
              <p className="px-2 text-xs font-medium text-slate-400">Signed in as</p>
              <p className="mt-1 truncate px-2 text-sm font-semibold text-white" title={email ?? undefined}>
                {email ?? 'Customer account'}
              </p>
              <button
                className="mt-3 flex min-h-11 w-full items-center gap-3 rounded-[10px] px-3 text-sm font-medium text-slate-300 transition hover:bg-white/8 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 disabled:cursor-not-allowed disabled:opacity-60"
        ...
        <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,rgba(71,118,255,0.08),transparent_28%),#0a0d12] px-4 pt-8 pb-28 sm:px-6 md:px-8 md:py-10 lg:px-12">
        ...
        <nav
          ...
          className="fixed inset-x-3 bottom-3 z-20 grid grid-cols-4 rounded-[16px] border border-white/10 bg-[rgba(17,22,31,0.92)] p-1.5 shadow-2xl backdrop-blur-xl md:hidden"
        >
          {navItems.map((item) => (
            <NavLink
              className={({ isActive }) =>
...
function PortalBrand({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`flex items-center gap-2.5 ${compact ? '' : 'px-2 py-1'}`}>
      ...
      <span className={`font-semibold tracking-[-0.02em] ${compact ? 'text-white' : 'text-slate-950'}`}>ISP Platform</span>
    </div>
  )
}
