...
export function AdminNotificationBell({ refreshKey }: { refreshKey: string }) {
  ...
  return <details className="relative"><summary className="relative grid size-11 cursor-pointer list-none place-items-center rounded-full border border-white/10 bg-[rgba(22,28,38,0.78)] text-slate-300 shadow-[0_4px_14px_rgba(0,0,0,0.16)] transition hover:border-white/15 hover:bg-white/10 hover:text-white active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0d12]" title="Admin notifications"><Bell aria-hidden="true" size={18} strokeWidth={1.8} /><span className="sr-only">Open admin notifications{feed?.unread_count ? `, ${feed.unread_count} unread` : ''}</span>{feed?.unread_count ? <span className="absolute -top-0.5 -right-0.5 grid min-h-4 min-w-4 place-items-center rounded-full border border-[#11161f] bg-blue-500 px-1 text-[9px] font-bold text-white">{feed.unread_count > 99 ? '99+' : feed.unread_count}</span> : null}</summary><div className="absolute right-0 z-50 mt-2 w-[min(23rem,calc(100vw-2rem))] overflow-hidden rounded-[14px] border border-white/10 bg-[rgba(17,22,31,0.98)] shadow-2xl backdrop-blur-xl md:right-auto md:left-0"><div className="flex items-center justify-between border-b border-white/8 px-4 py-3"><div><p className="font-semibold text-white">Operations alerts</p><p className="text-xs text-slate-500">Items that may need attention</p></div><Link className="text-xs font-semibold text-blue-300 hover:text-blue-200" to="/admin/notifications">View all</Link></div>{loadError ? <p className="p-4 text-sm text-red-300">Notifications could not be loaded.</p> : !feed ? <p className="p-4 text-sm text-slate-500">Loading notifications…</p> : feed.notifications.length === 0 ? <p className="p-4 text-sm text-slate-500">No action items right now.</p> : <ul className="max-h-96 divide-y divide-white/8 overflow-y-auto">{feed.notifications.map((item) => <li key={item.id}><button className={`w-full px-4 py-3 text-left hover:bg-white/5 ${item.read_at ? '' : 'bg-blue-500/8'}`} onClick={() => void openNotification(item)} type="button"><span className="block text-sm font-semibold text-white">{item.title}</span><span className="mt-1 block text-xs leading-5 text-slate-400">{item.message}</span><time className="mt-1 block text-[11px] text-slate-600" dateTime={item.created_at}>{timestampFormatter.format(new Date(item.created_at))}</time></button></li>)}</ul>}</div></details>
}
