...
export function CustomerNotificationsPage() {
  ...
  return <section className="w-full max-w-4xl"><CustomerPageHeader actions={feed?.unread_count ? <Button disabled={saving} onClick={() => void markAllRead()} variant="secondary"><CheckCheck aria-hidden="true" size={17} />{saving ? 'Updating…' : 'Mark all as read'}</Button> : null} description="Important account, service, and support updates." eyebrow="Customer portal" title="Notifications" /><div className="mt-8">{error && !feed ? <ErrorPanel message={error} title="Notifications unavailable" /> : !feed ? <PageSkeleton count={5} type="list" /> : feed.notifications.length === 0 ? <EmptyState description="Important account and service updates will appear here." title="No notifications yet" /> : <div className="overflow-hidden rounded-[18px] border border-slate-900/8 bg-white shadow-[0_18px_50px_rgba(18,25,38,0.06)]">{error ? <p className="border-b border-red-200 bg-red-50 p-4 text-sm text-red-700" role="alert">{error}</p> : null}<ul className="divide-y divide-slate-900/8">{feed.notifications.map((notification) => <li key={notification.id}><button className={`flex min-h-20 w-full items-start gap-4 p-5 text-left transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500 sm:p-6 ${notification.read_at ? '' : 'bg-blue-50/50'}`} onClick={() => void markRead(notification)} type="button"><span aria-hidden="true" className={`mt-1 size-2.5 shrink-0 rounded-full ${notification.read_at ? 'bg-slate-200' : 'bg-blue-600'}`} /><span className="min-w-0 flex-1"><span className="block font-semibold text-slate-950">{notification.title}</span><span className="mt-1 block text-sm leading-6 text-slate-600">{notification.message}</span><time className="mt-2 block text-xs text-slate-400" dateTime={notification.created_at}>{timestampFormatter.format(new Date(notification.created_at))}</time></span>{isSafeCustomerPath(notification.navigation_path) ? <span aria-hidden="true" className="text-slate-400">→</span> : null}</button></li>)}</ul></div>}</div></section>
}
