  ...
  if (submitted) {
    return (
      <AuthPanel>
        ...
        <Link className={buttonClassName({ className: 'mt-7 w-full', size: 'lg' })} to="/login">Return to sign in</Link>
      </AuthPanel>
    )
  }

  return (
    <AuthPanel backLabel="Back to sign in" backTo="/login">
      ...
      <form className="mt-7" noValidate onSubmit={(event) => void submitRequest(event)}>
        <AuthField autoComplete="email" autoFocus error={emailError ?? undefined} id="recovery-email" label="Email address" name="email" onChange={(event) => { setEmail(event.target.value); setEmailError(null) }} type="email" value={email} />
        <div className="mt-5">
        ...
        <Button className="mt-6 w-full" disabled={isSubmitting || !captchaToken} size="lg" type="submit">{isSubmitting ? <><LoadingSpinner size="sm" /> Sending...</> : 'Send reset instructions'}</Button>
      </form>
    </AuthPanel>
  )
}
