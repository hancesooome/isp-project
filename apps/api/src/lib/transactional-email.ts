...
export function buildTransactionalEmail(content: TransactionalEmailContent): {
  ...
  const text = [
    ...
    'This is an automated service message from ISP Platform. Please do not send passwords or payment details by email.',
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escapeHtml(content.headline)}</title></head><body style="margin:0;background:#f4f6f9;color:#111318;font-family:Arial,Helvetica,sans-serif"><div style="display:none;max-height:0;overflow:hidden;opacity:0">${escapeHtml(content.preheader)}</div><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6f9"><tr><td align="center" style="padding:32px 16px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:620px;background:#ffffff;border:1px solid #e4e7ec;border-radius:14px;overflow:hidden"><tr><td style="padding:22px 30px;border-bottom:1px solid #e4e7ec"><span style="display:inline-block;width:5px;height:20px;margin-right:9px;border-radius:4px;background:#4776ff;vertical-align:middle"></span><strong style="font-size:19px;letter-spacing:-0.3px;vertical-align:middle">ISP Platform</strong></td></tr><tr><td style="padding:34px 30px"><h1 style="margin:0 0 22px;color:#111318;font-size:26px;line-height:1.25;letter-spacing:-0.5px">${escapeHtml(content.headline)}</h1><p style="margin:0 0 16px;color:#344054;font-size:15px;line-height:1.7">${escapeHtml(greeting)}</p>${content.paragraphs.map((paragraph) => `<p style="margin:0 0 16px;color:#475467;font-size:15px;line-height:1.7">${escapeHtml(paragraph)}</p>`).join('')}${detailsHtml}${noticeHtml}${actionHtml}</td></tr><tr><td style="padding:22px 30px;border-top:1px solid #e4e7ec;background:#fafbfc;color:#667085;font-size:12px;line-height:1.6"><p style="margin:0 0 8px">Need help? <a href="${escapeHtml(supportUrl)}" style="color:#245eea;text-decoration:none">Contact customer support</a>.</p><p style="margin:0">This automated service message was sent by ISP Platform. Never send passwords or payment details by email.</p></td></tr></table></td></tr></table></body></html>`
